...
public class PlayView implements ControllerToView{
	private ViewToController playcontroller;
	...
	public void playGame() {
		...
		while(i<team.length) {
			...
			if(arr[0].equalsIgnoreCase(winningTeam) || arr[1].equalsIgnoreCase(winningTeam)) {
			    playcontroller.changePointsTable(winningTeam);
			    i++;
			}
		...
		playcontroller.changePosition();
		playcontroller.getsemiTeams();
